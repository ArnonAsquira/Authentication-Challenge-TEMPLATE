...
const { validateBodySchema, validateExistingUser, validateUser, authenticateToken } = require('../middlewares/validateSchema');
const { getUseres, REFRESHTOKENS, pushToUSERS, pushToINFORMTION } = require('../usersDB');
const jwt = require('jsonwebtoken');
const { JWT_ACCES_SECRET, JWT_REFRESH_SECRET } = require('../env');
const { generateAccesToken } = require('../utils/jwtFunction');

router.post('/register', validateBodySchema, validateExistingUser, async (req, res, next) => {
    const body = req.body;
    try {
        const hashedPassword = await bcrypt.hash(body.password, 10);
        body.password = hashedPassword;
        pushToUSERS(body);
        pushToINFORMTION({email: body.email, info: `${body.name} info`})
